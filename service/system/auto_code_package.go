...
func (s *autoCodePackage) Create(ctx context.Context, info *request.SysAutoCodePackageCreate) error {
	...
	return global.GVA_DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		for key, value := range creates { // key
			...
			if err != nil {
				return errors.Wrapf(err, "[filepath:%s]!", value)
			}
			var file *os.File
			...
			fmt.Printf("[template:%s][filepath:%s]!\n", key, value)
		}
...
func (s *autoCodePackage) All(ctx context.Context) (entities []model.SysAutoCodePackage, err error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "plugin!")
	}
	for i := 0; i < len(serverDir); i++ {
		if serverDir[i].IsDir() {
			serverPackage := model.SysAutoCodePackage{
				...
				Template:    "package",
				Label:       serverDir[i].Name() + "",
				Desc:        "" + serverDir[i].Name() + "",
	...
	for i := 0; i < len(pluginDir); i++ {
		if pluginDir[i].IsDir() {
			...
			pluginPackage := model.SysAutoCodePackage{
				...
				Template:    "plugin",
				Label:       pluginDir[i].Name() + "",
				Desc:        "" + pluginDir[i].Name() + "，v2",
				Module:      global.GVA_CONFIG.AutoCode.Module,
			}
...
func (s *autoCodePackage) templates(ctx context.Context, entity model.SysAutoCodePackage, info request.AutoCode, isPackage bool) (code map[string]string, asts map[string]ast.Ast, creates map[string]string, err error) {
	...
	if err != nil {
		return nil, nil, nil, errors.Wrapf(err, "[%s]!", templateDir)
	}
	for i := 0; i < len(templateDirs); i++ {
		...
		case "server":
			...
			if err != nil {
				return nil, nil, nil, errors.Wrapf(err, "[%s]!", second)
			}
			for j := 0; j < len(secondDirs); j++ {
				...
				if !secondDirs[j].IsDir() {
					...
					if ext != ".tpl" {
						return nil, nil, nil, errors.Errorf("[filpath:%s]!", three)
					}
				...
				case "api", "router", "service":
					...
					if err != nil {
						return nil, nil, nil, errors.Wrapf(err, "[%s]!", three)
					}
					for k := 0; k < len(threeDirs); k++ {
						...
						if threeDirs[k].IsDir() {
							return nil, nil, nil, errors.Errorf("[filpath:%s]!", four)
						}
				...
				case "model":
					...
					for k := 0; k < len(threeDirs); k++ {
						...
						if threeDirs[k].IsDir() {
							...
							if err != nil {
								return nil, nil, nil, errors.Wrapf(err, "[%s]!", four)
							}
							for l := 0; l < len(fourDirs); l++ {
								...
								if fourDirs[l].IsDir() {
									return nil, nil, nil, errors.Errorf("[filpath:%s]!", five)
								}
		...
		default:
			...
			return nil, nil, nil, errors.Errorf("[filpath:%s]!", second)
		}
	}
