...
func (userBetRecordService *UserBetRecordService) GetMyUserBetRecordInfoList(ctx context.Context, info apiReq.UserBetRecordSearch, uid uint) (list []api.UserBetRecord, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// db
	db := global.GVA_DB.Model(&api.UserBetRecord{})
	var userBetRecords []api.UserBetRecord
	//
	db = db.Joins("LEFT JOIN games ON user_bet_record.gametype = games.id")
	db = db.Where("user_code = ?", uid)
	err = db.Count(&total).Error
	if err != nil {
		return
	}
