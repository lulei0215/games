...
func (i *initAuthority) InitializeData(ctx context.Context) (context.Context, error) {
	...
	entities := []sysModel.SysAuthority{
		{AuthorityId: 888, AuthorityName: "", ParentId: utils.Pointer[uint](0), DefaultRouter: "dashboard"},
		{AuthorityId: 9528, AuthorityName: "", ParentId: utils.Pointer[uint](0), DefaultRouter: "dashboard"},
		{AuthorityId: 8881, AuthorityName: "", ParentId: utils.Pointer[uint](888), DefaultRouter: "dashboard"},
	}

	if err := db.Create(&entities).Error; err != nil {
		return ctx, errors.Wrapf(err, "%s!", sysModel.SysAuthority{}.TableName())
	}
	// data authority
